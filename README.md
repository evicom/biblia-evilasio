# ✝ Bíblia Sagrada

App web progressivo (PWA) com a Bíblia Sagrada completa. Funciona 100% offline após o primeiro acesso.

Por José Evilasio Marques

## 📖 Versões incluídas
- **ARC** — Almeida Revista e Corrigida
- **ACF** — Almeida Corrigida Fiel
- **NVI** — Nova Versão Internacional
- **AA** — Almeida Atualizada
- **KJA** — King James Atualizada

## ✨ Funcionalidades
- Leitura completa do Antigo e Novo Testamento
- Troca de versão **mantendo o livro e capítulo atuais**
- Busca por palavras em todos os livros
- Anotações por versículo (salvas localmente)
- **Modo escuro elegante** com destaque para palavras de Deus (azul) e de Jesus (vermelho)
- Ajuste de tamanho de fonte
- **Selecionar texto e compartilhar** (WhatsApp, Instagram, Telegram, Facebook, X)
- Compartilhamento de versículos
- **Botão de instalar como app (PWA)** com instruções para Android, iPhone e PC
- Funciona 100% offline (Service Worker cacheia até os arquivos .json)

## 🚀 Como publicar no GitHub Pages
1. Faça upload de todos os arquivos para um repositório no GitHub
2. Vá em **Settings → Pages**
3. Em **Source**, selecione `main` e a pasta `/ (root)`
4. Clique em **Save**
5. Aguarde alguns minutos — o app estará disponível em:
   `https://usuario.github.io/nome-preferencia/`

## 📁 Estrutura de arquivos
